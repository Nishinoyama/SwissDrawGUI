...
// JSONが存在しない、初期状態でのビルド
void SwissSystemTournament::build(){

    // ダミーのプレイヤーは、マッチング時のみ生成するようにする
    players.assign( PlayerNumber, Player() );
    playersPermutation.assign( PlayerNumber, nullptr );
    for (int i = 0; i < PlayerNumber; i++){
        players[i].id = i;
        playersPermutation[i] = &players[i];
    }
    availPlayerNumber = players.size();
...
void SwissSystemTournament::setPlayer(int id, std::string& playerName, int rating)
{
...
// JSONデータがあるときのビルド
void SwissSystemTournament::buildFromJSON() {
    ...
    for( int i = 0; i < PlayerNumber; i++ ) {
        ...
        if (!players[i].withdrew) availPlayerNumber++;
    }
...
void SwissSystemTournament::Matching() {
    ...
    bool dummy = availPlayerNumber % 2;
    int imagePlayerNumber = availPlayerNumber + dummy; // must be even number
    SortPlayers();
    ...
    // 28試合を限界とする。（28試合あれば、1000万人の試合でも十分な試合数）
    for (int ext = std::min(6, matchedRounds + 2); ext < 28; ext++) {
        std::vector<std::vector<int>> dp(imagePlayerNumber + 1, std::vector<int>(1llu << ext, 1e9));
        std::vector<std::vector<std::pair<int, int>>> 
            rb(imagePlayerNumber + 1, std::vector<std::pair<int, int>>(1llu << ext, {-1, -1}));
        dp[0][0] = 0;
        for (int ni = 0; ni < imagePlayerNumber; ni++) {
            for (int bi = 0; bi < 1 << ext; bi++) {
                // dp[ni][bi] からの変遷
                // ni = 上位ni+1人目からのマッチング調査
                // bi = 以下ext人のマッチング情報
                if (dp[ni][bi] == INF) continue; // 至らなかった変遷は飛ばす
                ...
                // ダミープレイヤーだったらマッチングを探してはならない
                if (ni == availPlayerNumber) {
                    break;
                }

                // マッチング済みでないならマッチングを探す。
                for (int pi = 0; pi < ext; pi++) {
                    // ni+pi+1+1人目とマッチングを考える。
                    int ppi = ni + pi + 1;
                    int bbi = (bi >> 1) | (1 << pi);
                    if (ppi > availPlayerNumber ) break;

                    // マッチング済みだったら、探しなおし
                    if ((bi >> 1) & (1 << pi)) continue;

                    // マッチ相手がダミー（プレイヤー情報を持たない）なら別処理           
                    if (ppi == availPlayerNumber) {
                        int dummyID = -1;
                        int cost = playersPermutation.at(ni)->points;
                        if (playersPermutation[ni]->matchedPlayerID.find(dummyID)
                        ...
                        int cost = playersPermutation.at(ni)->points - playersPermutation.at(ppi)->points;
        ...
        // dpの変遷からロールバックし、マッチング情報を入手
        // dp[imagePlayerNumber][0]からdp[0][0]戻る
        int rbn = imagePlayerNumber, rbb = 0;
        ...
        while( rbn != 0 || rbb != 0 ){
            ...
            if( rb[rbn][rbb].second ){
                ...
                if (li < availPlayerNumber && ri < availPlayerNumber) {
                    // ダミーがいない場合
                    ...
                    // ダミーがいる場合
                    playersPermutation[li]->opponent = nullptr;
                }
            }
            rbn--;
            rbb = tmp;
...
void SwissSystemTournament::InputMatchResult() {
    ...
    for ( const picojson::value& matchingValue : matchingList ){
        ...
        searched[leftID] = searched[rightID] = true;
    ...
    for ( Player& p : players ) {
        if (!searched[p.id]) {
            if (!p.withdrew) {
                MatchResult dummyResult(0, 0, 0, 0, 1, nullptr);
                p.pushMatchedResults(dummyResult);
            }
        }
    }
...
void SwissSystemTournament::CalculatePlayerState() {
    ...
    for (Player& player : players) {
        player.buildMatchResult(players);
    }
